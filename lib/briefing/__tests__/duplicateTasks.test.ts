...
describe('getDuplicateTasks', () => {
  ...
  it('filters by projectId when provided', () => {
    ...
    insertTask(db, { id: 't1', projectId: 'p1', title: 'Task A p1' })
    insertTask(db, { id: 't2', projectId: 'p1', title: 'Task B p1' })
    insertTask(db, { id: 't3', projectId: 'p2', title: 'Task A p2' })
    insertTask(db, { id: 't4', projectId: 'p2', title: 'Task B p2' })
    insertEmbedding(db, { projectId: 'p1', taskId: 't1', vector: [1, 0, 0] })
    insertEmbedding(db, { projectId: 'p1', taskId: 't2', vector: [1, 0, 0] })
    insertEmbedding(db, { projectId: 'p2', taskId: 't3', vector: [1, 0, 0] })
    insertEmbedding(db, { projectId: 'p2', taskId: 't4', vector: [1, 0, 0] })
    const out = getDuplicateTasks(db, { projectId: 'p1', threshold: 0.85 })
    expect(out.every(x => x.projectId === 'p1')).toBe(true)
    expect(out.length).toBeGreaterThan(0)
  })

  it('excludes pairs present in dedup_dismissals', () => {
    insertTask(db, { id: 't1', projectId: 'p1', title: 'Task A' })
    insertTask(db, { id: 't2', projectId: 'p1', title: 'Task B' })
    insertTask(db, { id: 't3', projectId: 'p1', title: 'Task C' })
    insertEmbedding(db, { projectId: 'p1', taskId: 't1', vector: [1, 0, 0] })
    insertEmbedding(db, { projectId: 'p1', taskId: 't2', vector: [1, 0, 0] })
    insertEmbedding(db, { projectId: 'p1', taskId: 't3', vector: [1, 0, 0] })
    // Dismiss the t1/t2 pair (lex order: t1 < t2)
    const now = new Date().toISOString()
    db.prepare(`INSERT INTO dedup_dismissals (project_id, a_task_id, b_task_id, dismissed_at) VALUES (?, ?, ?, ?)`).run('p1', 't1', 't2', now)
    const out = getDuplicateTasks(db, { threshold: 0.85 })
    // t1/t2 is dismissed; t1/t3 and t2/t3 remain
    expect(out.some(x => x.aTaskId === 't1' && x.bTaskId === 't2')).toBe(false)
    expect(out.length).toBeGreaterThan(0)
  })
})
