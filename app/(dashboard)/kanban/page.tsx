      ...
      <div className="flex flex-col gap-1.5">
        {cards.map((card, i) => (
          <div key={`${card.projectId}-${card.featureName}-${i}`}
            className={`bg-bg-primary border border-border-default rounded-lg px-3 py-2 ${card.stale ? 'opacity-50' : ''}`}>
            <div className="flex items-center justify-between">
              <span className="text-xs font-medium text-text-primary truncate">{card.featureName}</span>
              {card.auditStatus && <span className="text-xs ml-1">{AUDIT_ICONS[card.auditStatus]}</span>}
            </div>
            <div className="flex items-center gap-2 mt-0.5">
              <span className="text-[10px] text-text-muted">{card.projectName}</span>
              {card.stale && <span className="text-[10px] text-text-muted">{'\u23F8'} stale</span>}
            </div>
          </div>
        ))}
        {cards.length === 0 && (
          <div className="text-[10px] text-text-faint text-center py-4 border border-dashed border-border-default rounded-lg">Empty</div>
        )}
      </div>
    </div>
  )
    ...
    <>
      <div className="mb-6">
        <h1 className="text-lg font-semibold text-text-primary flex items-center gap-2">
          <Layout size={18} className="text-accent-blue" /> Pipeline Board
        </h1>
